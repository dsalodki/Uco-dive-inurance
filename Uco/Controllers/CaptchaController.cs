...
namespace Uco.Controllers
{
    ...
    public class CaptchaController : Controller
    {
        ...
        public JsonResult ValidateInvisibleCaptcha(string InvisibleCaptchaValue)
        {
            bool b = InvisibleCaptchaValue == "";
            if (!b) return Json(string.Empty, JsonRequestBehavior.AllowGet);
            else return Json(true, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var expectedHash = LS.CurrentHttpContext.Session["CaptchaHash"];
        ...
        //internal static bool IsValidCaptchaValue()
        //{
        //    string CaptchaValue = System.Web.HttpRequest.current.Form["CaptchaValue"];
        //    return IsValidCaptchaValue(CaptchaValue.ToUpper());
        //}

        //internal static bool IsValidInvisibleCaptchaValue()
        //{
        //    string InvisibleCaptchaValue = System.Web.HttpRequest.current.Form["InvisibleCaptchaValue"];
        //    return InvisibleCaptchaValue == "";
        //}

        internal static bool IsInvisibleCaptchaValid(string InvisibleCaptchaValue)
        {
            return InvisibleCaptchaValue == "";
        }

        internal static bool IsCaptchaValid(string CaptchaValue)
        {
            return IsValidCaptchaValue(CaptchaValue.ToUpper());
        }

        internal static bool IsGoogleReCaptchaValid(string recaptcha)
        {
            var result = false;
            if (!string.IsNullOrWhiteSpace(recaptcha))
            {
                string URI = "https://www.google.com/recaptcha/api/siteverify";
                string myParameters = $"secret=6LfGNLwUAAAAAP1pM6UVCbs0jz-Iyty-UPMxz1AP&response={recaptcha}";
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    {
                        var apiresponse = JsonConvert.DeserializeObject<GoogleReCaptchaAPIResponse>(htmlResult);
